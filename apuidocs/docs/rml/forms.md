...
`value`{:.attr} = cdata (CN)
: For types of `text`, `password` and `range`, this is used as the initial value of the element. For the `radio` and `checkbox` types this is used as the value that is submitted if the input is checked. For `<option>`{:.tag} elements, this is the value that is submitted by the parent `<select>`{:.tag}, if it is the selected option.
