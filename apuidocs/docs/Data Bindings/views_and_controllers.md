...
*Implementation note.* Internally, the XML parser uses a special parsing rule whenever the `data-for` attribute is encountered, providing all the children of the current element as raw HTML text to the data view, which is later used for creation of each item in the data array.
...
Sets the element's inner HTML to the evaluated expression.
