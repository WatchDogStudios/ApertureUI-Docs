...
The `Rml::Property` structure is defined in `<RmlUi/Core/Property.h>`. Below is a subset of the class, listing some of its useful members:
...
* `KEYWORD` values should be requested as int types. Keyword values are stored as integers for speed; to check what the value means, you can compare it to the constant values defined in `<RmlUi/Core/StyleSheetKeywords.h>`. For custom keyword properties, see below.
...
User-defined properties can be added to the global style sheet specification, so you can attach any values you'd like to your elements. This is done through the `Rml::StyleSheetSpecification` class (included through `RmlUi/Core.h` or `RmlUi/Core/StyleSheetSpecification.h`).
