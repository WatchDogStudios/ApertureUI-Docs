...
The `<template>` element has two uses, to define a template and inject a template inline into an existing HTML documents.
...
`name` = cdata (CI)
: The name of the template. Must be unique. Is used by other HTML documents to reference the template.
