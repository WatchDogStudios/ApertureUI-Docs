...
In RCSS, resolution is always specificied using a number (the scaling factor) followed by the `x` unit.
...
sets the `width` property to '25%' and the `min-width` property to '55px' on the `div`{:.tag} element. 