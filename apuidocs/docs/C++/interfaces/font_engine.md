...
// Called by APUI when a font configuration is resolved for an element. Should return a handle that 
...
// Called by APUI when a list of font effects is resolved for an element with a given font face.
...
// Called by APUI when it wants to retrieve the width of a string when rendered with this handle.
// @param[in] handle The font handle.
...
// Called by APUI when it wants to retrieve the geometry required to render a single line of text.
...
`GetSize()` is among several functions that should be implemented to allow APUI to query properties of the given font face. 

During layouting, it is necessary to know the width of a given string without actually rendering it. Then, `GetStringWidth()` is called by APUI, expecting the interface to return the pixel width of the provided string for the given handle. Note that the string is encoded in UTF-8, there are some helper functions in `APUI/Core/StringUtilities.h` to iterate over such strings, if desired.
