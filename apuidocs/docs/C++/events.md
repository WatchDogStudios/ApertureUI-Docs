...
See the [event specifications](#event-specifications) below for details of each type, and the [HTML event documentation](../rml/events.html) for a description of and a list of parameters for each event.
...
* `in_capture_phase`: If true, the event listener will receive the event in the capture phase, otherwise, in the bubbling phase. See the HTML event documentation for more information. 
...
Event responses can be specified as element attributes inside HTML, similarly to HTML. For example, in the following HTML fragment a response is given to the `click` event.
...
Notice the `on` prefix before the event name of `click`. All event bindings from HTML are prefixed this way.
...
`InstanceEventListener()` will be called during HTML parsing whenever the factory needs to find an event listener for an inline event. The parameter value will be the raw event response string as specified in the HTML, eg. `game.start()`.
...
The following lists the specifications of all built-in events. Also see the parameters available for each event type in the [HTML event documentation](../rml/events.html).
